...
function Profile() {
  ...
  const fetchhis=async()=>{
    try {
      setData(false)
      const res=await axios.get('http://localhost:3001/fetchhistory',{headers: {
        'Authorization': `Bearer ${token}`
      }
    }) 
    setDataA(res.data)
    console.log(res.data)
    setLastFourItems(res.data.slice(-4));
    setData(true)
  } catch (error) {
    setData(false)
    console.log(error)
...
function UserTable({ user }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 flex">
      <aside className="bg-gradient-to-br from-gray-800 to-gray-900 -translate-x-96  inset-0 z-50 w-80 h-ull my-4 ml-4   rounded-xl transition-transform duration-300 xl:translate-x-0">

            <div className="container w-full h-full    bg-white  shadow-lg    transform   duration-200 easy-in-out">
                <div className=" h-32 overflow-hidden" >
                    <img className="  w-full h-28" src="https://www.cpb.bank/media/cfxoee52/shaka-hero-image.png" alt="" />
                </div>
                <div className="flex justify-center px-5  -mt-12">
                    <img className="h-32 w-32 bg-white p-2 rounded-full   " src="https://cdn-icons-png.freepik.com/512/9967/9967422.png" alt="" />

                </div>
                <div className=" ">
                    <div className="text-center px-14">
                        <h2 className="text-gray-800 text-3xl font-bold">{user.Fname+user.Lname}</h2>
                        <Link className="text-gray-400 mt-2 hover:text-blue-500" to="https://www.instagram.com/immohitdhiman/" target="BLANK()">{user.UPIID}</Link>
      ...
      <div className="p-4 w-full">
        ...
        <div className="mt-12">
          ...
          <div className="mb-4 h-screen flex flex-col gap-6 overflow-x-hidden overflow-y-scroll">
  <div className="relative flex flex-col bg-clip-border rounded-xl overflow-x-hidden overflow-y-scroll bg-white text-gray-700 shadow-md">
    <div className="relative bg-clip-border rounded-xl overflow-hidden  bg-transparent text-gray-700 shadow-none m-0 flex items-center justify-between p-6">
      <div>
       
        <h6 className="block antialiased tracking-normal pt-4 font-sans text-xl font-semibold leading-relaxed text-black mb-1">
        <strong>Transaction History</strong>  
        </h6>
      </div>
   
    </div>
    <div className="p-6 px-10 w-full pt-0 pb-2">
      <table className="w-full  table-auto">
        <thead>
          <tr>
  ...
  <tr key={item.time}>
    ...
    <td className="py-3 px-5 border-b border-blue-gray-50">
      <div className="flex items-center gap-4">
        <p
          className={`block antialiased font-sans text-sm leading-normal font-bold ${
            item.sender === userToken ? 'text-red-500' : item.receiver === userToken ? 'text-green-500' : 'text-blue-gray-900'
          }`}
        >
          {item.ammount}
        </p>
